...
WINDOW_NAME    = 'Open Document Labeling'
TRACKBAR_IMG   = 'Page'
TRACKBAR_CLASS = 'Class'

annotation_formats = {'PASCAL_VOC' : '.xml', 'YOLO_darknet' : '.txt', 'Real_data' : 'real.txt'}
...
def yolo_format(ann_path, class_index, point_1, point_2, width, height):
    # name + path + page
    doc_name = StorageUtil.get_file_name(ann_path)
    parts = doc_name.split("-page")
    page = parts[len(parts)-1].split(".")[0]
    doc_name = doc_name.replace("-page" + page, "")
    doc_name = StorageUtil.replace_file_type(doc_name, constants.LABELED_DATA_FILE_EXT)
    doc_name = constants.LABELED_DATA_FILE_PATH + doc_name
    ...
    items = map(str, [doc_name, page, class_index, width, height,
                      point_1[0], point_2[0], point_1[1], point_2[1]])
    return ';'.join(items)


def real_format(class_index, point_1, point_2, width, height):
    x_min = point_1[0]
    y_min = point_1[1]
    x_max = point_2[0]
    y_max = point_2[1]
    items = map(str, [class_index, x_min, x_max, y_min, y_max, width, height])
    return ' '.join(items)
...
def edit_bbox(obj_to_edit, action):
    ...
    for path in bboxes_to_edit_dict:
        ...
        for ann_path in get_annotation_paths(path, annotation_formats):
            ...
            if 'real.txt' in ann_path:
                # edit YOLO file
                with open(ann_path, 'r') as old_file:
                    lines = old_file.readlines()

                real_line = real_format(class_index, (xmin, ymin), (xmax, ymax), width, height)
                ...
                with open(ann_path, 'w') as new_file:
                    for line in lines:
                        if line != real_line + '\n':
                            ...
                            new_real_line = real_format(new_class_index, (xmin, ymin), (xmax, ymax), width, height)
                            new_file.write(new_real_line + '\n')
                        elif 'resize_bbox' in action:
                            new_real_line = real_format(class_index, (new_x_left, new_y_top), (new_x_right, new_y_bottom), width, height)
                            new_file.write(new_real_line + '\n')

            elif '.txt' in ann_path:
                # edit YOLO file
                with open(ann_path, 'r') as old_file:
                    lines = old_file.readlines()

                yolo_line = yolo_format(ann_path, class_index, (xmin, ymin), (xmax, ymax), width, height)
                ...
                with open(ann_path, 'w') as new_file:
                    for line in lines:
                        if line != yolo_line + '\n':
                            ...
                            new_yolo_line = yolo_format(ann_path, new_class_index, (xmin, ymin), (xmax, ymax), width, height)
                            new_file.write(new_yolo_line + '\n')
                        elif 'resize_bbox' in action:
                            new_yolo_line = yolo_format(ann_path, class_index, (new_x_left, new_y_top), (new_x_right, new_y_bottom), width, height)
                            new_file.write(new_yolo_line + '\n')
...
def save_bounding_box(annotation_paths, class_index, point_1, point_2, width, height):
    for ann_path in annotation_paths:
        if 'real.txt' in ann_path:
            line = real_format(class_index, point_1, point_2, width, height)
            append_bb(ann_path, line, '.txt')
        elif '.txt' in ann_path:
            line = yolo_format(ann_path, class_index, point_1, point_2, width, height)
            append_bb(ann_path, line, '.txt')
        elif '.xml' in ann_path:
...
if __name__ == '__main__':
    ...
    for f in os.listdir(constants.PDF_LABEL_INPUT_PATH):
        # test if it is a pdf
        if f.endswith(".pdf"):
            f_path = os.path.join(constants.PDF_LABEL_INPUT_PATH, f)
            pages = convert_from_path(f_path, 300)
            pdf_file = f[:-4]
            if len(pages) > 1:
                if not os.path.exists(constants.LABEL_INPUT_PATH + "%s-page0.jpg" % pdf_file):
                    for page in pages:
                        page.save(constants.LABEL_INPUT_PATH + "%s-page%d.jpg"
                                  % (pdf_file, pages.index(page)), "JPEG")
            elif not os.path.exists(constants.LABEL_INPUT_PATH + "%s-page0.jpg" % pdf_file):
                pages[0].save(constants.LABEL_INPUT_PATH + "%s-page0.jpg" % pdf_file, "JPEG")

    for f in os.listdir(constants.LABEL_INPUT_PATH):
        f_path = os.path.join(constants.LABEL_INPUT_PATH, f)
    ...
    while True:
        ...
        if WITH_QT:
            # f if window gets closed then quit
            if cv2.getWindowProperty(WINDOW_NAME, cv2.WND_PROP_VISIBLE) < 1:
                break

    cv2.destroyAllWindows()

    # transform labeling results
    df = pd.DataFrame({"file": [], "page": [],
                       "format": [], "width": [], "height": [],
                       "bounding_box": []})
    for f in os.listdir(constants.LABEL_OUTPUT_PATH):
        if f.endswith(".txt"):
            file = {"format": "Custom",
                    "bounding_box": []}
            media_box = {}
            print(constants.LABEL_OUTPUT_PATH + f)
            with open(constants.LABEL_OUTPUT_PATH + f, 'r',
                      encoding='windows-1252') as myfile:
                for line in myfile:
                    if line != "":
                        items = line.split(";")
                        if int(items[2]) == 0:  # just adding tables
                            # TODO support more: here limited for tablebank compatibility
                            if "file" not in file:
                                # file info
                                file["file"] = items[0]
                                ...
                                # coordinates
                                file["width"] = media_box["x1page"] - media_box["x0page"]
                                file["height"] = media_box["y1page"] - media_box["y0page"]

                            bounding_box = [int(int(items[5])*xf), int(int(items[6])*xf),
                                            int(int(items[7])*yf), int(int(items[8].rstrip())*yf)]
                            bounding_box_tb = [bounding_box[0], bounding_box[2],
                                               bounding_box[0], bounding_box[3],
                                               bounding_box[1], bounding_box[3],
                                               bounding_box[1], bounding_box[2]]
                            file["bounding_box"].append([bounding_box_tb])
            if file["bounding_box"]:
                file["file"] = file["file"][3:]
                df = df.append(file, ignore_index=True)
    df.to_csv(constants.PDF_LABEL_INPUT_PATH + "../gt_labeled_" +
              str(time.localtime()[0]) + str(time.localtime()[1]).zfill(2) + str(time.localtime()[2]).zfill(2) +
              str(time.localtime()[3]).zfill(2) + str(time.localtime()[4]).zfill(2) + ".csv",
              sep=";", index=0)
